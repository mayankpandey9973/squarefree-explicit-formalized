...
theorem a_asymp_A {P : Globals} {S : Scale P} {a : ℤ} {r : ℕ}
  ...
  have ha_le_d : a0 ≤ d0 := by
    ...
    have hden_lt : d0 ^ 2 * (d0 + a0) ^ 2 < 16 * D ^ 2 * a0 ^ 2 := by
      have hd0sq : d0 ^ 2 ≤ (2 * D) ^ 2 := by nlinarith only [hd2, hDpos, hd0pos]
      have hsumsq : (d0 + a0) ^ 2 < (2 * a0) ^ 2 := by nlinarith only [hsum_lt, hsum_pos, ha0pos]
    ...
    have hRfun_gt : X / (16 * D) < X * a0 ^ 3 / (d0 ^ 2 * (d0 + a0) ^ 2) := by
      ...
      have hge2 : X / (16 * D) ≤ X * a0 / (16 * D ^ 2) := by
        ...
        nlinarith only [mul_pos (mul_pos hXpos hDpos) (by linarith : (0:ℝ) < a0 - D), hXpos, hDpos]
      rw [heq] at hlt1
      linarith
    ...
    have hXD_gt : 17 * R < X / (16 * D) := by
      ...
      have hH3 : (272 : ℝ) * S.Ω ^ 3 < P.H ^ 3 := by
        nlinarith only [hHge, hΩpos, hHpos, mul_pos hΩpos hΩpos, sq_nonneg (P.H - 10 * S.Ω),
      ...
      nlinarith only [mul_lt_mul_of_pos_left hH3 hfac, hfac]
  ...
  have hden_lo : D ^ 4 ≤ d0 ^ 2 * (d0 + a0) ^ 2 := by
    have h1 : D ^ 2 ≤ d0 ^ 2 := by nlinarith only [hd1, hDpos, hd0pos]
    have h2 : D ^ 2 ≤ (d0 + a0) ^ 2 := by nlinarith only [hsum_lo, hDpos, hsum_pos]
    nlinarith only [h1, h2, pow_pos hDpos 2]
  have hden_hi : d0 ^ 2 * (d0 + a0) ^ 2 ≤ 64 * D ^ 4 := by
    have h1 : d0 ^ 2 ≤ (2 * D) ^ 2 := by nlinarith only [hd2, hDpos, hd0pos]
    have h2 : (d0 + a0) ^ 2 ≤ (4 * D) ^ 2 := by nlinarith only [hsum_hi, hDpos, hsum_pos]
    nlinarith only [h1, h2, pow_pos hDpos 2, sq_nonneg d0, sq_nonneg (d0 + a0)]
  ...
  have ha0cube_lo : A ^ 3 / 73 ≤ a0 ^ 3 := by
    ...
    have hkey : A ^ 3 ≤ a0 ^ 3 * 73 := by
      have heq : R * D ^ 4 = A ^ 3 * X := by
        ...
        field_simp at this ⊢
        linarith [this]
      nlinarith only [hXa3, hXpos, heq]
    linarith [hkey]
  -- cube-monotone: a0³ ≥ A³/73 ≥ (A/5)³ ⟹ a0 ≥ A/5
  have hlower : A / 5 ≤ a0 := by
    nlinarith only [ha0cube_lo, hApos, ha0pos, sq_nonneg (a0 - A / 5), sq_nonneg (a0 + A / 5),
  ...
  have ha0cube_hi : a0 ^ 3 ≤ 1088 * A ^ 3 := by
    have heq : R * D ^ 4 = A ^ 3 * X := by
      ...
      field_simp at this ⊢
      linarith [this]
    nlinarith only [hXa3_hi, hXpos, heq]
  have hupper : a0 ≤ 11 * A := by
    nlinarith only [ha0cube_hi, hApos, ha0pos, sq_nonneg (a0 - 11 * A), sq_nonneg (11 * A - a0),
      mul_pos ha0pos ha0pos, mul_pos hApos hApos]
  exact ⟨hlower, hupper⟩
...
theorem dtilde_asymp_D {P : Globals} {S : Scale P} {a r : ℝ}
  ...
  have hd2_lt_w : d ^ 2 < w := by
    have : d ^ 2 < d * (d + a) := by nlinarith only [hdpos, ha0]
    linarith [hprod, this]
  have hw_le : w ≤ 310 * D ^ 2 := by
    nlinarith only [hw2_hi, sq_nonneg (w - 310 * D ^ 2), hwpos, hDpos, pow_pos hDpos 2,
      mul_pos hwpos hwpos]
  have hupper : d ≤ 18 * D := by
    nlinarith only [hd2_lt_w, hw_le, sq_nonneg (d - 18 * D), hdpos, hDpos, pow_pos hDpos 2,
      mul_pos hdpos hdpos]
  ...
  have hd_ge_a : a ≤ d := by
    ...
    have hw_lt : w < 2 * a ^ 2 := by
      have : d * (d + a) < a * (2 * a) := by nlinarith only [hdpos, ha0, hcon]
      nlinarith only [hprod, this]
    have hw2_lt : w ^ 2 < 4 * a ^ 4 := by
      nlinarith only [hw_lt, hwpos, ha0, sq_nonneg a, mul_pos ha0 ha0]
    ...
    have hXar : X < 4 * a * r := by
      ...
      nlinarith only [hcross, ha3pos, ha0]
    -- `4 a r ≤ 4·(11A)·(16R) = 704 A R`
    have hbound : 4 * a * r ≤ 704 * (A * R) := by
      nlinarith only [ha_hi, hr_hi, ha0, hr0, hApos, hRpos,
  ...
  have hw_le_2d2 : w ≤ 2 * d ^ 2 := by
    have : d * (d + a) ≤ 2 * d ^ 2 := by nlinarith only [hdpos, hd_ge_a]
    linarith [hprod, this]
  have hw_ge : D ^ 2 / 45 ≤ w := by
    nlinarith only [hw2_lo, sq_nonneg (w - D ^ 2 / 45), hwpos, hDpos, pow_pos hDpos 2,
      mul_pos hwpos hwpos]
  have hlower : D / 10 ≤ d := by
    nlinarith only [hw_le_2d2, hw_ge, sq_nonneg (d - D / 10), hdpos, hDpos, pow_pos hDpos 2,
      mul_pos hdpos hdpos]
  exact ⟨hlower, hupper⟩
