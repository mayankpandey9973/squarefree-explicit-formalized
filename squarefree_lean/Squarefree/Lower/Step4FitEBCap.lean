...
private theorem fitEB_tailB (hG1 : 1 ≤ P.G) (hU1 : 1 ≤ P.U) (hΔ1 : 1 ≤ S.Δ)
    (C K : ℝ) (hC0 : 0 ≤ C) (hK : K ≤ P.U^3) (g u : ℕ) (hg : g ≤ 15) (hu : u ≤ 87) :
    K*C*P.G^g*P.U^u/S.Ω^27 ≤ C*S.Δ*P.G^15*P.U^90/S.Ω^27 := by
  have hΩpos := S.Ω_pos
  have hGpos := P.G_pos
  have hUpos := P.U_pos
  calc K*C*P.G^g*P.U^u/S.Ω^27
      ≤ P.U^3*C*P.G^15*P.U^87/S.Ω^27 := by
        gcongr ?_*C*?_*?_/S.Ω^27
        · exact pow_le_pow_right₀ hG1 hg
        · exact pow_le_pow_right₀ hU1 hu
    _ = C*1*P.G^15*P.U^90/S.Ω^27 := by ring
    _ ≤ C*S.Δ*P.G^15*P.U^90/S.Ω^27 := by
        gcongr C*?_*P.G^15*P.U^90/S.Ω^27
...
theorem step4_fitEB_cap
  ...
  have h3t : 2*(C*(P.H/S.Δ)*(S.Δ^2*P.G^15*P.U^90/(P.H*S.Ω^27)))
      = 2*(C*S.Δ*P.G^15*P.U^90/S.Ω^27) := by field_simp
  have hGpos := P.G_pos
  have hUpos := P.U_pos
  have hΩpos := S.Ω_pos
  have hΔpos := S.Δ_pos
...
theorem step4_fit_cE_B
  ...
  have hblock : (0:ℝ) ≤ C*(P.H/S.Δ)*(S.Δ^2*P.G^15*P.U^90/(P.H*S.Ω^27)) :=
    mul_nonneg (mul_nonneg (by linarith) (div_nonneg P.H_pos.le S.Δ_pos.le))
      (div_nonneg
        (mul_nonneg (mul_nonneg (sq_nonneg _) (pow_nonneg P.G_pos.le 15))
  ...
  rw [show cEhyb P S a ℓ₁ ℓ₂ gap
    ...
    show (8:ℝ) * C * (P.H/S.Δ) * (S.Δ^2*P.G^15*P.U^90/(P.H*S.Ω^27))
      = 8 * (C * (P.H/S.Δ) * (S.Δ^2*P.G^15*P.U^90/(P.H*S.Ω^27))) from by ring]
