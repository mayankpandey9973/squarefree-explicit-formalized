...
/-!
# §7 N9′ family lemmas C: the `B_i` double differences of `f₂` (md 1618–22)
...
/-- **N9′ family C** (md 1618–22): the double-differenced `f₂` expansion at a `ξ`-shifted
point, graded `m ≤ 2`, generic in the steps `(p, q)` and shift `ξ`. -/
theorem build_B_exp (RE : Sec7RaExpData P S W a Ph j)
    ...
    ∀ m ≤ 2, ∀ r ∈ sec7_rWin S W,
      |iteratedDeriv m (fun t =>
        ...
        sec7_cExp * ((p : ℝ) * q * (S.T₂ / S.R ^ 2) * sec7_relErr P S +
  ...
  -- numerics
  have haprod : |sec7_aprod (3/4) (m + 3)| ≤ 3 := by
    interval_cases m <;> norm_num [sec7_aprod]
  have hpowle : ((2:ℝ) * sec7_cWin) ^ (m + 3) ≤ (2 * sec7_cWin) ^ 5 :=
  ...
  have hcoef : 2 * (|RE.c₂| * |sec7_aprod (3/4) (m + 3)| * (2 * sec7_cWin) ^ (m + 3)) ≤
    ...
    have h12 : |RE.c₂| * |sec7_aprod (3/4) (m + 3)| ≤ 12 := by nlinarith
    have h1 : |RE.c₂| * |sec7_aprod (3/4) (m + 3)| * (2 * sec7_cWin) ^ (m + 3) ≤
        12 * (2 * sec7_cWin) ^ 5 :=
      le_trans (mul_le_mul_of_nonneg_right h12 p3)
        (mul_le_mul_of_nonneg_left hpowle (by norm_num))
    have h2 : (0:ℝ) ≤ 12 * (2 * sec7_cWin) ^ 5 := by positivity
    calc 2 * (|RE.c₂| * |sec7_aprod (3/4) (m + 3)| * (2 * sec7_cWin) ^ (m + 3)) ≤
        2 * (12 * (2 * sec7_cWin) ^ 5) := by linarith
      _ ≤ sec7_cExp := by norm_num [sec7_cWin, sec7_cExp]
  have hkey1 : C * q * p ≤
      sec7_cExp * ((p:ℝ) * q * (S.T₂ / S.R ^ 2) * sec7_relErr P S) / S.R ^ m := by
    ...
    calc C * q * p
        = sec7_cExpIn * ((p:ℝ) * q * (S.T₂ / S.R ^ 2) * sec7_relErr P S / S.R ^ m) := by
          ...
          field_simp
          try ring
      _ ≤ sec7_cExp * ((p:ℝ) * q * (S.T₂ / S.R ^ 2) * sec7_relErr P S / S.R ^ m) :=
          mul_le_mul_of_nonneg_right hIn hmono
      _ = sec7_cExp * ((p:ℝ) * q * (S.T₂ / S.R ^ 2) * sec7_relErr P S) / S.R ^ m := by
          ring
  have hkey2 : (p:ℝ) * q * (B * (2 * hSv)) ≤
      sec7_cExp * ((p:ℝ) * q * hSv * S.T₂ / S.R ^ 3) / S.R ^ m := by
    ...
    calc (p:ℝ) * q * (B * (2 * hSv))
      ...
      _ ≤ sec7_cExp * ((p:ℝ) * q * hSv * S.T₂ / S.R ^ (m + 3)) :=
          mul_le_mul_of_nonneg_right hcoef hmono
      _ = sec7_cExp * ((p:ℝ) * q * hSv * S.T₂ / S.R ^ 3) / S.R ^ m := by
          rw [pow_add]
          field_simp
  calc C * q * p + (p:ℝ) * q * (B * (2 * hSv))
      ≤ sec7_cExp * ((p:ℝ) * q * (S.T₂ / S.R ^ 2) * sec7_relErr P S) / S.R ^ m +
        sec7_cExp * ((p:ℝ) * q * hSv * S.T₂ / S.R ^ 3) / S.R ^ m := by
        linarith [hkey1, hkey2]
    _ = sec7_cExp * ((p:ℝ) * q * (S.T₂ / S.R ^ 2) * sec7_relErr P S +
