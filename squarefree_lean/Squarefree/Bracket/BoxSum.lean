...
theorem prop_7_1 : ∃ C : ℝ, 0 < C ∧
    ∀ (P : Globals) (S : Scale P) (W : ℝ), AdmissibleW P S W →
      ∀ (ftil dBreve dBreve' : ℝ → ℝ) (gfun : ℤ → ℝ → ℝ),
        (∀ (j : ℤ) (r : ℝ),
          gfun j r = dBreve (ftil r + j) - dBreve' (ftil r + j) * Int.fract (ftil r)) →
        ∀ (δ₀ : ℝ), 0 < δ₀ →
          δ₀ ≤ S.Δ ^ 5 / (P.H ^ 3 * P.G ^ 2 * S.Ω ^ 2) + S.Δ ^ 2 / (P.H ^ 2 * P.G * S.A) →
          ∀ (j : ℤ) (cj : ℝ), 0 < cj → (|j| : ℝ) ≤ cj * (1 + P.H / S.A ^ 2) →
            (((Finset.Icc ⌈S.R⌉ ⌊2 * S.R⌋).filter
                (fun r => distInt (gfun j (r : ℝ)) ≤ δ₀)).card : ℝ) ≤ C * (S.R / W) := by
  sorry -- STUB: prop_7_1

/-- **Prop 7.3** (writeup 1993–2000): `#ℛ_a ≪ (1+H/A²)·R/W`. `Ra` is the §3 set (mapped into 𝒟_a). -/
theorem prop_7_3 : ∃ C : ℝ, 0 < C ∧
    ∀ (P : Globals) (S : Scale P) (a : ℤ), 0 < a → ∀ (W : ℝ), AdmissibleW P S W →
      ∀ (Ra : Finset ℕ) (dStar : ℕ → ℤ), (∀ r ∈ Ra, inDa P.X P.H a (dStar r)) →
