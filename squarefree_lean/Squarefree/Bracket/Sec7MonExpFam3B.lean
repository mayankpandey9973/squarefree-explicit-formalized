...
/-!
# §7 N9′ family lemmas D at m=3 (Φ″ endgame): triple differences `B₀₃`, `Δf₃`

The m=3-specific siblings of `build_B03_exp`/`build_d3f3_exp` (Sec7MonExpFam3.lean), on the
looser `sec7_cExp3` budget.  At `m = 3` the triple-difference diff-depth is `ord 6`, so these
consume the ORDER-6 residual chain `e₂D_deriv6`/`e₂D_bound6`, `e₃D_deriv6`/`e₃D_bound6`
(budget `sec7_cExpIn6`) through the order-6 generic core `build_diff3_generic6`.  The m ≤ 2
path (`build_B03_exp`/`build_d3f3_exp`, on `sec7_cExp`) is left untouched.
-/
...
/-- **N9′ family D, order-6 generic core** (md 1623–33): the m=3 sibling of
`build_diff3_generic`, consuming the order-6 residual chain (`hed : ∀ m < 6`,
`heb : ∀ m ≤ 6` against `sec7_cExpIn6`), with the falling-factorial bound to order 7 and the
window power `(2cWin)^{7−α}`, concluding on the `sec7_cExp3` budget for `m ≤ 3`. -/
private theorem build_diff3_generic6 {P : Globals} {S : Scale P} {W : ℝ} {h₁ h₂ h₃ : ℤ}
    ...
    (hed : ∀ m < 6, ∀ x ∈ sec7_rWinWide S W, HasDerivAt (e m) (e (m + 1) x) x)
    (heb : ∀ m ≤ 6, ∀ x ∈ sec7_rWinWide S W,
      |e m x| ≤ sec7_cExpIn6 * (T / S.R ^ m) * rel)
    (hap : ∀ k ≤ 7, |sec7_aprod α k| ≤ Ap)
    (hKb : (2 * sec7_cWin) ^ ((7:ℝ) - α) ≤ Kb)
    (hcoefb : |c| * Ap * Kb ≤ sec7_cExp3) :
    ∀ m ≤ 3, ∀ r ∈ sec7_rWin S W,
      |iteratedDeriv m (fun t => diff3 (h₁:ℝ) h₂ h₃ f t -
          c * sec7_aprod α 3 * sec7_Pprod h₁ h₂ h₃ * (T / S.R ^ 3) *
            (t / S.R) ^ (α - 3)) r| ≤
        sec7_cExp3 * (sec7_Pprod h₁ h₂ h₃ * (T / S.R ^ 3) * rel +
  ...
  set C : ℝ := sec7_cExpIn6 * (T / S.R ^ (m + 3)) * rel with hC
  have hCnn : 0 ≤ C := by
    rw [hC]
    exact mul_nonneg (mul_nonneg sec7_cExpIn6_pos.le
  ...
  have hMbd : ∀ x ∈ sec7_rWinWide S W, |M (m + 4) x| ≤ B := by
    ...
    have hexple : (2 * sec7_cWin) ^ (((m + 4 : ℕ) : ℝ) - α) ≤ Kb := by
      ...
      have hm7 : ((m + 4 : ℕ) : ℝ) ≤ 7 := by
        have : ((m + 4 : ℕ) : ℝ) = (m:ℝ) + 4 := by push_cast; ring
        rw [this]
        have : (m:ℝ) ≤ 3 := by exact_mod_cast hm
        linarith
      linarith
  ...
  -- numerics
  have hkey1 : C * h₃ * h₂ * h₁ ≤
      sec7_cExp3 * (PP * (T / S.R ^ 3) * rel) / S.R ^ m := by
    have hIn : sec7_cExpIn6 ≤ sec7_cExp3 := by norm_num [sec7_cExpIn6, sec7_cExp3]
    ...
    calc C * h₃ * h₂ * h₁
        = sec7_cExpIn6 * (PP * (T / S.R ^ 3) * rel / S.R ^ m) := by
          rw [hC, hPPval, pow_add]
          field_simp
          try ring
      _ ≤ sec7_cExp3 * (PP * (T / S.R ^ 3) * rel / S.R ^ m) :=
          mul_le_mul_of_nonneg_right hIn hmono
      _ = sec7_cExp3 * (PP * (T / S.R ^ 3) * rel) / S.R ^ m := by ring
  have hkey2 : PP * (B * hSv) ≤ sec7_cExp3 * (PP * hSv * T / S.R ^ 4) / S.R ^ m := by
    ...
    calc PP * (B * hSv)
      ...
      _ ≤ sec7_cExp3 * (PP * hSv * T / S.R ^ (m + 4)) :=
          mul_le_mul_of_nonneg_right hcoefb hmono
      _ = sec7_cExp3 * (PP * hSv * T / S.R ^ 4) / S.R ^ m := by
          rw [pow_add]
          field_simp
  calc C * h₃ * h₂ * h₁ + PP * (B * hSv)
      ≤ sec7_cExp3 * (PP * (T / S.R ^ 3) * rel) / S.R ^ m +
        sec7_cExp3 * (PP * hSv * T / S.R ^ 4) / S.R ^ m := by linarith [hkey1, hkey2]
    _ = sec7_cExp3 * (PP * (T / S.R ^ 3) * rel +
        PP * hSv * T / S.R ^ 4) / S.R ^ m := by ring

/-- **N9′ family D, `B₀₃` instance at m=3** (md 1623–27): the triple-differenced `f₂`
expansion, graded `m ≤ 3`, on the `sec7_cExp3` budget. -/
theorem build_B03_exp3 (RE : Sec7RaExpData P S W a Ph j)
    (hh₁ : 1 ≤ h₁) (hh₂ : 1 ≤ h₂) (hh₃ : 1 ≤ h₃)
    (hW : 0 < W) (hpad : 6 * (W + W ^ 2 + W ^ 4) ≤ S.R / 288)
    (hshift : 3 * sec7_hSum h₁ h₂ h₃ ≤ 3 * (W + W ^ 2 + W ^ 4)) :
    ∀ m ≤ 3, ∀ r ∈ sec7_rWin S W,
  ...
  intro m hm r hr
  have hgen := build_diff3_generic6 hh₁ hh₂ hh₃ hW hpad hshift (f := Ph.f2D 0) (e := RE.e₂D)
    (c := RE.c₂) (α := (3:ℝ)/4) (T := S.T₂) (Ap := 39) (Kb := (2 * sec7_cWin) ^ 7)
    (sec7_T₂_pos S) (by norm_num) (sec7_relErr_pos P S) RE.e₂D_zero RE.e₂D_deriv6
    RE.e₂D_bound6
    ...
    (by
      have h1 := Real.rpow_le_rpow_of_exponent_le
        ...
        (show (7:ℝ) - 3/4 ≤ ((7:ℕ):ℝ) by norm_num)
      rwa [Real.rpow_natCast] at h1)
    (by
      have hc := RE.c₂_hi
      have hKnn : (0:ℝ) ≤ (2 * sec7_cWin) ^ 7 := by norm_num [sec7_cWin]
      have h1 : |RE.c₂| * 39 * (2 * sec7_cWin) ^ 7 ≤ 4 * 39 * (2 * sec7_cWin) ^ 7 := by
...
/-- **N9′ family D, `Δf₃` instance at m=3** (md 1628–33): the triple-differenced `f₃`
expansion, graded `m ≤ 3`, on the `sec7_cExp3` budget. -/
theorem build_d3f3_exp3 (RE : Sec7RaExpData P S W a Ph j)
    (hh₁ : 1 ≤ h₁) (hh₂ : 1 ≤ h₂) (hh₃ : 1 ≤ h₃)
    (hW : 0 < W) (hpad : 6 * (W + W ^ 2 + W ^ 4) ≤ S.R / 288)
    (hshift : 3 * sec7_hSum h₁ h₂ h₃ ≤ 3 * (W + W ^ 2 + W ^ 4)) :
    ∀ m ≤ 3, ∀ r ∈ sec7_rWin S W,
      |iteratedDeriv m (fun t =>
        ...
        sec7_cExp3 * (sec7_Pprod h₁ h₂ h₃ * (S.T₃ / S.R ^ 3) * sec7_relErrF P S +
          sec7_Pprod h₁ h₂ h₃ * sec7_hSum h₁ h₂ h₃ * S.T₃ / S.R ^ 4) / S.R ^ m := by
  intro m hm r hr
  have hgen := build_diff3_generic6 hh₁ hh₂ hh₃ hW hpad hshift (f := Ph.f3D j 0) (e := RE.e₃D)
    (c := 3 * RE.c₁ * RE.c₂) (α := -(1:ℝ)/4) (T := S.T₃) (Ap := 319)
    (Kb := 10 ^ 24)
    (sec7_T₃_pos S) (by norm_num) (sec7_relErrF_pos P S) RE.e₃D_zero RE.e₃D_deriv6
    RE.e₃D_bound6
    (by intro k hk; interval_cases k <;> norm_num [sec7_aprod])
    (by
      have h1 : ((7:ℝ) - -(1:ℝ)/4) = ((29:ℕ):ℝ) / 4 := by push_cast; norm_num
      rw [h1]
    ...
    (by
      ...
      have h1 : |3 * RE.c₁ * RE.c₂| * 319 * (10:ℝ) ^ 24 ≤ 48 * 319 * (10:ℝ) ^ 24 := by
        nlinarith
      refine le_trans h1 ?_
      norm_num [sec7_cExp3])
    m hm r hr
