...
/-!
# §7 N9′ family lemmas A–B: `f₁` shift and first difference (md 1607–17)
...
/-- **N9′ family A** (md 1607–12): the shifted `f₁` expansion, graded `m ≤ 2`. -/
theorem build_f1_exp (RE : Sec7RaExpData P S W a Ph j)
  ...
  -- numerics
  have haprod : |sec7_aprod (-1) (m + 2)| ≤ 24 := by
  ...
  have hcoef : |RE.c₁| * |sec7_aprod (-1) (m + 2)| * (2 * sec7_cWin) ^ (m + 3) ≤
    ...
    have h96 : |RE.c₁| * |sec7_aprod (-1) (m + 2)| ≤ 96 := by nlinarith
    have h1 : |RE.c₁| * |sec7_aprod (-1) (m + 2)| * (2 * sec7_cWin) ^ (m + 3) ≤
        96 * (2 * sec7_cWin) ^ 5 :=
      le_trans (mul_le_mul_of_nonneg_right h96 p3)
...
generic in the step `h ∈ {h₁, h₂, h₃}`. -/
theorem build_d1f1_exp (RE : Sec7RaExpData P S W a Ph j)
  ...
  have hkey2 : B * (h:ℝ) * hSv ≤
    ...
    calc B * (h:ℝ) * hSv
      ...
      _ ≤ sec7_cExp * ((h:ℝ) * hSv * S.T₁ / S.R ^ (m + 2)) :=
          mul_le_mul_of_nonneg_right hcoef hmono
